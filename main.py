...
def process_and_shorten(urls_list):
    ...
    for url in urls_list:
        ...
        time.sleep(0.05)  # Небольшая пауза, чтобы Short.io не блокировал запросы
    return short_urls
...
# --- Обработка прикрепленных файлов (.txt) ---
...
def handle_document(message):
    ...
    try:
        ...
        if not raw_urls:
            bot.edit_message_text("❌ В файле не найдено ни одной ссылки.", chat_id=message.chat.id, message_id=status_msg.message_id)
            return
        ...
        bot.send_document(
            ...
            caption=f"✅ Готово! Успешно обработано: {len(short_links)} ссылок."
        )
        bot.delete_message(message.chat.id, status_msg.message_id)

    except Exception as e:
        bot.edit_message_text(f"❌ Ошибка при обработке файла: {e}", chat_id=message.chat.id, message_id=status_msg.message_id)
...
def handle_text(message):
    ...
    if not raw_urls:
        bot.reply_to(message, "❌ Не найдено ни одной валидной ссылки.")
        return
...
if __name__ == "__main__":
    keep_alive()
    print("✅ Бот запущен в классическом режиме!")
